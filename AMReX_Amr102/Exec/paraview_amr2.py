...
def create_single_image(pltfile_name, image_prefix):

  #### disable automatic camera reset on 'Show'
  paraview.simple._DisableFirstRenderCameraReset()

  # create a new 'AMReX/BoxLib Grid Reader'
  plt00050 = AMReXBoxLibGridReader(registrationName='plt00050', FileNames=pltfile_name)
  ...
  plt00050_1 = AMReXBoxLibParticlesReader(registrationName='plt00050', FileNames=pltfile_name)
  ...
  ## ===
  # Use number suffix to name image

  suffix = pltfile_name[3:]
  print(suffix)
  imagefile_name = image_prefix + suffix + ".png"
  print(imagefile_name)

  # save animation
  renderView1.Update()
  SaveScreenshot(imagefile_name, renderView1, ImageResolution=[1478, 1471])
...
def convert_images_to_mp4(glob_pattern, output_movie_base):
    # use ffmpeg to convert the avi movie into an animated gif
    ffmpeg_imgs_to_mp4 = 'ffmpeg -framerate 30 -pattern_type glob -i "{}" -c:v libx264 -preset veryslow -crf 0 -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -hide_banner {}.mp4'.format(glob_pattern, output_movie_base)
    subprocess.run(ffmpeg_imgs_to_mp4, shell=True)

def convert_mp4_to_gif(input_movie, output_gif_base):
    # use ffmpeg to convert the avi movie into an animated gif
    ...
    ffmpeg_convert_to_gif = 'ffmpeg -i {} -vf "fps=30,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 {}.gif'.format(input_movie, output_gif_base)
...
if __name__ == "__main__":
    ...
    for pltfile in PlotFiles:
        create_single_image(pltfile, "img")

    convert_images_to_mp4("img*.png", "amrex_102")

    convert_mp4_to_gif("amrex_102.mp4","amrex_102")
